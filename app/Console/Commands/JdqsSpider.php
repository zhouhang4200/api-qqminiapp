...
class JdqsSpider extends Command
{
    protected $url = '';
    /**
     ...
     * @var string
     */
    protected $signature = 'jdqs:spider';
        ...
        try {
            ...
            while (true) {
                if ($this->url) {
                    ...
                    $original_url = config('spider.spider_url.game.jdqs'); // 改第一处
                }
...
//                dd($documents);
                ...
                if (is_array($documents) && count($documents) > 0) {
                    foreach ($documents as $k => $document) {
                        try {
                            ...
                            // 数据库是否有相同记录，有则跳过
                            $video = Video::where('category_id', config('spider.category.jdqs')) // 改第五处
                            ...
                            $insert[] = [
                                ...
                                'status'       => 0,
                                'category_id'  => config('spider.category.jdqs'), // 改第二处
                                'title'        => $title,
                                'thumb'        => $thumb,
                                'original_url' => $url,
                                'url'          => '',
                            ...
                            myLog('jdqs_original_url_spider_error', ["【" . $e->getLine() . "】" . $e->getMessage()]); // 改第三处
                            continue;
                        }
                    }
                    // 写入数据库
            ...
            myLog('jdqs_original_url_spider_error', ["【" . $e->getLine() . "】" . $e->getMessage()]); // 改第四处
        }
    }
}
