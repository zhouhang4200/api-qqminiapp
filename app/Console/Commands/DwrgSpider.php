...
class DwrgSpider extends Command
{
    protected $url = '';
    /**
     ...
     * @var string
     */
    protected $signature = 'dwrg:spider';
        ...
        try {
            ...
            while (true) {
                if ($this->url) {
                    ...
                    $original_url = config('spider.spider_url.game.dwrg'); // 改第一处
                }
                ...
                if (is_array($documents) && count($documents) > 0) {
                    foreach ($documents as $k => $document) {
                        try {
                            ...
                            // 数据库是否有相同记录，有则跳过
                            $video = Video::where('category_id', config('spider.category.dwrg')) // 改第五处
                            ...
                            $insert[] = [
                                ...
                                'status'       => 0,
                                'category_id'  => config('spider.category.dwrg'), // 改第二处
                                'title'        => $title,
                                'thumb'        => $thumb,
                                'original_url' => $url,
                                'url'          => '',
